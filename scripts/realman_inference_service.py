...
class ParquetToJointState(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(JointState, "rm_joint_topic", 10)
        ...
        # 声明参数：Parquet 文件路径
        episode_name = "episode_000049"
        self.declare_parameter(
            "parquet_file",
            f"/home/robot/datasets2/lerobot/isaaclab_pick-place-demo-brannew_640_480.50/data/chunk-000/{episode_name}.parquet",
        )
        self.declare_parameter("publish_rate", 15.0)  # 发布频率 (Hz)

        # 获取参数值
        self.parquet_file = self.get_parameter("parquet_file").get_parameter_value().string_value
        ...
        try:
            ...
            # 开始发布数据
            self.timer = self.create_timer(1.0 / self.publish_rate, self.publish_next_state)
    ...
    def publish_next_state(self):
        ...
        if isinstance(action_data, str):
            ...
            try:
                ...
                action_data = [float(x) for x in action_data.strip("[]").split(",")]
            except:
        ...
        self.get_logger().info(f"发布关节状态 #{self.current_index}: {action_data[:]}")
...
class CustomRos2Subscriber(Node):
    def __init__(self, node_name="custom_subscriber"):
        ...
        self.sub_joint = self.create_subscription(
            JointState, "/rm_joint_topic", self.joint_topic_callback, 10
        )
        self.sub_joint  # 防止未使用变量警告
        # 订阅相机信息
        self.sub_pos_front = self.create_subscription(
            Image, "/camera2/camera2/color/image_raw", self.front_image_topic_callback, 10
        )
        # 订阅相机信息
        self.sub_pos_side = self.create_subscription(
            Image, "/camera1/camera1/color/image_raw", self.side_image_topic_callback, 10
        )
        # 订阅相机信息
        self.sub_pos_wrist = self.create_subscription(
            Image, "/camera3/camera3/color/image_rect_raw", self.wrist_image_topic_callback, 10
        )
        self.sub_joint  # 防止未使用变量警告
        ...
        self.obs = {
            ...
            "video.side_view": np.random.randint(0, 256, (1, 480, 640, 3), dtype=np.uint8),
            "video.wrist_view": np.random.randint(0, 256, (1, 480, 640, 3), dtype=np.uint8),
    ...
    def joint_topic_callback(self, msg):
        ...
        with lock:
            ...
            self.obs["state.single_arm"] = np.array(msg.position[0:6]).reshape(1, -1)
            self.obs["state.gripper"] = np.array(msg.position[6]).reshape(1, -1)
            # print(msg.position)
    ...
    def side_image_topic_callback(self, msg):
        ...
        with lock:
            ...
            img_reshaped = img_array.reshape(msg.height, msg.width, 3)
            img_batched = np.expand_dims(img_reshaped, axis=0)
            self.obs["video.side_view"] = img_batched

    def wrist_image_topic_callback(self, msg):
        print("""接收到 /camera3/camera3/color/image_rect_raw 消息后的处理逻辑""")
        with lock:
            from datetime import datetime
            ...
            img_reshaped = img_array.reshape(msg.height, msg.width, 3)
            img_batched = np.expand_dims(img_reshaped, axis=0)
            self.obs["video.wrist_view"] = img_batched
...
def custom_logic_thread(jointcommand_node, node):
    ...
    while rclpy.ok():
        ...
        msg = JointState()
        msg.name = ["joint1", "joint2", "joint3", "joint4", "joint5", "joint6", "finger1_joint"]
...
if __name__ == "__main__":
    ...
    if args.server:
        ...
        custom_t = threading.Thread(
            target=custom_logic_thread,
            args=(
                jointcommand_node,
                subscriber_node,
            ),
